...
func (bc *BoltClient) OpenBoltDb() {
	var err error
	bc.boltDB, err = bolt.Open("atlas.db", 0600, nil)
	if err != nil {
		log.Fatal(err)
	}
}
...
// Creates an "AtlasBucket" in our BoltDB. It will overwrite any existing bucket of the same name.
func (bc *BoltClient) initializeBucket() {
	bc.boltDB.Update(func(tx *bolt.Tx) error {
		_, err := tx.CreateBucket([]byte("AtlasBucket"))
		if err != nil {
